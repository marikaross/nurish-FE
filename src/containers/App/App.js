      ...
      return (
        <div className="app">
          ...
          <Route
            exact
            path="/formulas"
            render={() => <FormulaContainer animateLogo={this.animateLogo} />}
          />
          <Route
            exact
            path="/formulas/:id"
            render={({ match }) => {
              const formula = this.props.formulas.find(
                formula => formula.id === parseInt(match.params.id, 10)
              );
              return (
                <div>
                  <DetailsCard
                    formula={formula}
                    animateLogo={this.animateLogo}
                  />
                </div>
              );
            }}
