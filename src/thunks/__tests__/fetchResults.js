...
describe('fetchResults' ,() => {
  ...
  it('should call dispatch with the isLoading action', () => {
    const thunk = fetchResults(mockUrl)
    thunk(mockDispatch)
  ...
  it('should dispatch filterResultFetchDataSuccess if all is well', async () => {
    ...
    const thunk = fetchResults(mockUrl)
    await thunk(mockDispatch)
    expect(mockDispatch).toHaveBeenCalledWith(filterResultFetchDataSuccess(mockOneFormula))
   })
})